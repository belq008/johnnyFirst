...
public class UserSettingsDbManager extends AbsDBManager implements IDBManager<UserSetting>{
	...
	public static UserSettingsDbManager getInstance(Context context){
		mctx=context;
		if(instance == null){
			instance = new UserSettingsDbManager(context);
		}
		return instance;
	}
	...
	public UserSetting getById(String id) {
		...
		try {
			sql = "select * from " + DBTable.UserSetting.TABLE_NAME
					+ " where _account = "+id;
			cursor = getInstance(mctx).sqliteDB().rawQuery(sql, null);
	...
	public int insert(UserSetting t) {
		...
		try {
			...
			result = (int) getInstance(mctx).sqliteDB().insert(DBTable.UserSetting.TABLE_NAME, null, values);
	...
	public int update(UserSetting t) {
		...
		try {
			...
			values.put(DBTable.UserSetting.COLUMN_MSGVOICE, t.getMsgVoice());
			values.put(DBTable.UserSetting.COLUMN_TOKEN, t.getToken());
			result = (int) getInstance(mctx).sqliteDB().update(DBTable.UserSetting.TABLE_NAME, values, "_account = ?", new String[]{t.getAccount()});
